...
./scripts/deploy.sh            # ./mvnw clean package → ince WAR → WildFly'a deploy + marker bekle
...
```bash
./mvnw spring-boot:run -Dspring-boot.run.profiles=local   # http://localhost:2525/spring-wildfly-arch
```
